...
A consuming system MUST include an Access token in the HTTP authorisation header as an OAuth Bearer Token (as outlined in [RFC 6749](https://tools.ietf.org/html/rfc6749)). This will be in the form of a JSON Web Token (JWT) as defined in [RFC 7519](https://tools.ietf.org/html/rfc7519).  
...
Currently, as the new [NHS Digital authentication service](https://developer.nhs.uk/apis/national-authentication/) is not yet in place, the process outlined below should be followed:-  
### Process ###
NHS Digital authorised CDSS provider and consumer systems will be created as objects in the directory of the NHS Digital Authorisation server.  
...
The Authorisation server will issue a JWT on receiving a consumer system request and the JWT will contain details of the approved group for that requestor.  
The consuming system can then include the JWT in the HTTP authorisation header when a request is made to the provider.  
