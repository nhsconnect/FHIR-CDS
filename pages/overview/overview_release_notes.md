...
* Changed Encounter.location.location to use the CareConnect.Location profile
* Removed usage guidance of CareConnect.ReferralRequest profile
* Updated new example for Questionnaire resource Quantity - BPM
* Updated error response guidance when a failure status is used and what must be supplied with it
* Updated guidance on the Encounter Report Referral request as the Appointment resource references ReferralRequest.supportingInfo
* Guidance for CarePlan.supportingInfo has been updated
* Select Service Definition Example Error has been fixed
* ServiceDefinition resource - Representing Age guidance updated
* ValueSets - Composition.type valueset has been updated to use the same Snomed code as CDA document type
* API CheckServices – ReferralRequest has been updated and the evaluate has been replaced with check-services wording
* Practitioner Role Resource has been added in as it is referenced in the specification
* List.intent has been removed from List resource
* Update guidance on the use of Message for Encounter Report
* Rebrand Usage Guide to reflect name change from CDS API to UEC Connect
* Author Clinical Safety Case Report
* Updated guidance on the use of Organisation and Practitioner in the Encounter Report
